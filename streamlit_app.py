    ...
    with st.spinner("Thinking..."):
        try:
            response = client.chat.completions.create(
                model=MODEL_NAME,
                messages=[
                    {"role": "system", "content": "You are Nurse Zoey Zoe, a compassionate nurse. Only general wellness education — never diagnose. Refer to Greg for workouts or Fred for homes."},
                    *st.session_state.chat_history["zoey"]
                ],
                max_tokens=800,
                temperature=0.7
