...
<style>
    ...
    .agent-card { text-align: center; padding: 1rem; border-radius: 15px; background: rgba(255,255,255,0.95); box-shadow: 0 4px 8px rgba(0,0,0,0.1); margin: 15px; }
    .bubble { background: white; border-radius: 20px; padding: 8px 15px; display: inline-block; margin-bottom: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
...
# Hero image - original that always worked
...
with cols[0]:
    ...
    st.markdown('<div class="bubble"><strong>Fred</strong></div>', unsafe_allow_html=True)
...
with cols[1]:
    st.markdown("<div class='agent-card'>", unsafe_allow_html=True)
    st.markdown('<div class="bubble"><strong>Greg</strong></div>', unsafe_allow_html=True)
...
with cols[2]:
    st.markdown("<div class='agent-card'>", unsafe_allow_html=True)
    st.markdown('<div class="bubble"><strong>Dr. Zoey Zoe</strong></div>', unsafe_allow_html=True)
    st.image("https://image.shutterstock.com/image-vector/cute-asian-female-doctor-cartoon-600w-2267904077.jpg", width=150)  # First animated Asian nurse avatar
...
if st.session_state.selected_agent == "fred":
    ...
    # Fixed group wellness image (the one that always worked)
